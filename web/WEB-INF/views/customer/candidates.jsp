...
<div class="page-head">
    <div>
        <span class="eyebrow">STEP 2 OF 3 · 选择工程师</span>
        <h1>附近可接单工程师</h1>
        <p>已根据 <b><c:out value="${repairRequest.device_name}"/></b> · <b><c:out value="${repairRequest.fault_name}"/></b> · <b><c:out value="${repairRequest.area_name}"/></b> 和可预约时间为你筛选。</p>
    </div>
...
<div class="dispatch-strip">
    <div><b>当前故障</b><span><c:out value="${repairRequest.fault_name}"/></span></div>
    <div><b>服务区域</b><span><c:out value="${repairRequest.area_name}"/></span></div>
    <div><b>派单模式</b><span>用户自主选择 + 综合排序</span></div>
    ...
    <c:when test="${empty candidates}">
        <div class="card text-center py-5">
            <div class="card-body">
                ...
                <a class="btn btn-primary" href="${pageContext.request.contextPath}/customer/request">重新填写报修</a>
            </div>
        </div>
    </c:when>
    <c:otherwise>
                    ...
                    <article class="card h-100 border-0 shadow-sm candidate-card ${s.index == 0 ? 'recommended' : ''}">
                        <div class="card-body p-4 d-flex flex-column">
                            <div class="d-flex gap-3 candidate-identity">
                                ...
                                <div class="min-w-0">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span class="rating"><c:choose><c:when test="${x.completed_count == 0}">暂无评分</c:when><c:otherwise>★ <c:out value="${x.average_rating}"/></c:otherwise></c:choose></span>
                                    </div>
                                    <p class="candidate-bio"><c:out value="${x.bio}"/></p>
                                </div>
                            </div>
                            <c:if test="${s.index == 0}">
                                <div class="recommendation-note">综合推荐：评分、履约率和可预约时间更匹配</div>
                            </c:if>
                            ...
                            <div class="candidate-metrics">
                                ...
                                <div><b><c:choose><c:when test="${x.completed_count == 0}">—</c:when><c:otherwise><c:out value="${x.fulfillment_rate}"/>%</c:otherwise></c:choose></b><span>履约率</span></div>
