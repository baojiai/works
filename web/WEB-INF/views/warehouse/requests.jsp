...
<div class="page-head"><div><span class="eyebrow">PART REQUESTS</span><h1>配件申请审核与出库</h1><p>审核通过时锁定库存；实际领取后再从总量和锁定量中出库。</p></div></div>
<c:choose>
                ...
                <article class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
                            <div>
                                <span class="badge ${r.status == 'PENDING' ? 'text-bg-warning' : (r.status == 'APPROVED' ? 'text-bg-primary' : (r.status == 'ISSUED' ? 'text-bg-success' : (r.status == 'REJECTED' ? 'text-bg-danger' : (r.status == 'COMPLETED' ? 'text-bg-secondary' : 'text-bg-light'))))}"><c:out value="${r.status}"/></span>
