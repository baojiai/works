...
<div class="page-head"><div><span class="eyebrow">INVENTORY</span><h1>库存与流水</h1><p>当前总量始终等于可用量与锁定量之和，所有变化均可追溯。</p></div></div>
<div class="row g-3">
    <c:forEach items="${inventory}" var="x">
        <div class="col-12 col-md-6 col-xl-4">
            <article class="card border-0 shadow-sm h-100 inventory-card">
                <div class="card-body p-4 d-flex flex-column">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        ...
                        <span class="badge ${x.available_quantity <= x.warning_threshold ? 'text-bg-danger' : 'text-bg-success'}">${x.available_quantity <= x.warning_threshold ? '库存预警' : '库存正常'}</span>
                    </div>
                    ...
                    <button type="button" class="btn btn-outline-secondary btn-sm w-100 mt-auto" data-bs-toggle="modal" data-bs-target="#stockModal${x.part_id}">调整库存</button>
                </div>
            </article>
    ...
    <div class="card-body p-4">
        ...
        <div class="table-responsive">
            <table class="table table-sm table-hover align-middle mb-0">
                ...
                <c:if test="${empty flows}"><tr><td colspan="7" class="text-center text-secondary py-4">暂无库存流水</td></tr></c:if>
                </tbody>
            </table>
        </div>
    </div>
