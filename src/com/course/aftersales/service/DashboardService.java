    ...
    public Map<String,Object> summary(SessionUser user) throws Exception {
        ...
        try (Connection c = Database.open()) {
            if ("ENGINEER".equals(user.getRole())) {
                result.put("我预约的服务", count(c,"SELECT COUNT(*) c FROM appointment WHERE customer_id=? AND status IN ('BOOKED','PENDING_RESCHEDULE')",user.getId()));
                result.put("我收到的工单", count(c,"SELECT COUNT(*) c FROM repair_order WHERE engineer_id=? AND order_status IN ('PENDING_VISIT','REPAIRING','WAITING_PARTS','REWORK')",user.getId()));
                result.put("可接单时段", count(c,"SELECT COUNT(*) c FROM engineer_schedule WHERE engineer_id=? AND status='AVAILABLE' AND service_date>=CURRENT_DATE",user.getId()));
